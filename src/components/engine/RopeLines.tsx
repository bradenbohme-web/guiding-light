...
function buildRopeGeometry(rope: RopeParams, rigging: LaserRiggingParams, boomAngle: number) {
  ...
  for (const segment of rope.segments) {
    const ordered = [segment.startPoint, ...segment.throughPulleys, segment.endPoint].filter(Boolean).filter((id, i, arr) => i === 0 || id !== arr[i - 1]);
  ...
  return new THREE.TubeGeometry(curve, Math.max(16, allPoints.length * 2), Math.max(0.0018, rope.diameter * 0.45), 8, false);
}

function RopeMesh({ rope, rigging, boomAngle, showWireframe, highlight, onClick }: {
  rope: RopeParams; rigging: LaserRiggingParams; boomAngle: number; showWireframe: boolean; highlight: boolean; onClick?: () => void;
}) {
  ...
  return (
    <mesh geometry={geometry} onClick={(e) => { e.stopPropagation(); onClick?.(); }}>
      <meshStandardMaterial color={rope.color} roughness={0.68} metalness={0.08} wireframe={showWireframe} emissive={emissive} emissiveIntensity={highlight ? 0.5 : 0} />
    </mesh>
  );
}
  ...
  return (
    <group>
      {rigging.ropes.map((rope, i) => (
        <RopeMesh key={rope.id} rope={rope} rigging={rigging} boomAngle={boomAngle} showWireframe={showWireframe}
          highlight={highlightRopeId === rope.id} onClick={() => onRopeClick?.(i)} />
      ))}
    </group>
  );
}
